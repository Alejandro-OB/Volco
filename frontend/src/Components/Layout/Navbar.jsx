...
function Navbar() {
  ...
  // Enlaces principales reducidos
  const navLinks = [
    { to: '/clientes', label: 'Clientes', icon: Users },
    { to: '/cuentas', label: 'Cuentas', icon: Wallet },
    { to: '/servicios', label: 'Servicios', icon: Truck },
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 h-[72px] bg-white/80 backdrop-blur-md border-b border-slate-100 z-[1000] flex items-center justify-between px-6 md:px-12 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)]">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="cursor-pointer shrink-0 transition-transform duration-300 hover:scale-105" onClick={() => navigate('/clientes')}>
          <img src={logoVolco} alt="Volco" className="h-14 md:h-20 w-auto object-contain" />
        </div>
      </div>
        ...
        {navLinks.map((link) => (
          <NavLink
            ...
            className={({ isActive }) => `
              ...
              ${isActive ? 'bg-white text-[#f58d2f] shadow-sm ring-1 ring-slate-200/50' : 'text-slate-500 hover:text-slate-900 hover:bg-white/50'}
            `}
          >
            {({ isActive }) => (
              <>
      ...
      <div className="relative" ref={dropdownRef}>
        <button 
          ...
          className={`flex items-center gap-3 pl-2 pr-4 py-2 rounded-2xl border transition-all group ${
            dropdownOpen ? 'bg-white border-[#f58d2f] shadow-lg shadow-orange-50' : 'bg-slate-50 border-slate-100 hover:bg-white hover:border-slate-200'
          }`}
        >
