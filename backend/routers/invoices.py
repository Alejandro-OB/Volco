...
from models import Invoice, InvoiceCreate, Provider, InvoiceCustomization
...
def get_invoice(id: int, session: SessionDep, token: TokenDep):
...
def get_invoice_pdf(invoice_id: int, session: SessionDep, provider: Provider = Depends(get_provider_current)):
    ...
    customization = session.exec(
        ...
        .where(InvoiceCustomization.apply_to_all_accounts)
    ).first()
