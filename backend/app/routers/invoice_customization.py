...
async def create_invoice_customization(
    ...
    signature_url = fallback_signature_url
    ...
    data_dict = {
        ...
        "page_size": page_size,
        "service_account_id": None if apply_to_all_accounts else service_account_id,
        "logo_url": logo_url,
...
async def update_invoice_customization(
    ...
    fields = [
        ...
        "include_bank_info", "page_size", "service_account_id"
    ]
    for field in fields:
        value = locals()[field]
        if value is not None:
            # If apply to all accounts is forced, drop local service account dependency
            if field == 'apply_to_all_accounts' and value is True:
                customization.service_account_id = None
